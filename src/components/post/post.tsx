...
const PostCompo = ({ postCode }: PostProps) => {
          ...
          ? {
              ...
              comments: prev.comments
                .map((comment) => {
                  ...
                  if (comment.replies?.some((reply) => reply.commentCode === code)) {
                    return {
                      ...comment,
                      replies: comment.replies.filter((reply) => reply.commentCode !== code),
                      replyCount: (Number(comment.replyCount) - 1).toString(),
                    };
                  }
                  return comment;
              ...
              commentCount:
                prev.comments.some((c) => c.commentCode === code)
  ...
  const handleLikePost = async (postCode: string) => {
    try {
      ...
      toast({
        variant: "destructive",
        description: error.response?.data?.message || "Failed to like the post!",
      });
    }
  };
  ...
  const onchangeTab = async (tabType: TabType) => {
    ...
    if (tabType === "cmt" && (postData.comments === null || postData.comments?.length === 0)) {
      try {
        setPostData((prev) => (prev ? { ...prev, isLoadingComments: true } : prev));
        ...
        setPostData((prev) => (prev ? { ...prev, isLoadingComments: false } : prev));
        toast({
          variant: "destructive",
            ...
            case "cmt":
              return (
                <ScrollArea
                  ...
                  data-scroll-ignore
                >
                  <PostHeader post={postData} onchangeTab={onchangeTab} />
                  <PostComments
                    post={postData}
